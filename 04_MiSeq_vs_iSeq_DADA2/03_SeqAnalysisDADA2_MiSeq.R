####
#### DADA2 analysis of MiSeq-style fastq files
#### 2020.7.21 Ushio
#### R 4.0.2
####
...
output_folder <- "03_SeqAnalysisDADA2_MiSeqOut"
...
#library(Biostrings); packageVersion("Biostrings") # 2.56.0, 2020.6.25
source("00_Helpers/DADA2Helpers.R")

# Load sequence reads
path <- paste0(getwd(), "/02_DemultiplexedOut_MiSeqStyle")
...
# Visualize errors
ggsave("03_SeqAnalysisDADA2_MiSeqOut/errF_MiSeq.pdf", plotErrors(errF, nominalQ = T), width = 10, height = 10)
ggsave("03_SeqAnalysisDADA2_MiSeqOut/errR_MiSeq.pdf", plotErrors(errR, nominalQ = T), width = 10, height = 10)
...
writeLines(capture.output(sessionInfo()),
           paste0("03_SessionInfo/SessionInfo_MiSeq", output_folder, "_", substr(Sys.time(), 1, 10), ".txt"))
write.csv(seqtab_nochim, "seqtab_nochim_miseq.csv", row.names = F)

system("rm -r 02_DemultiplexedOut_MiSeqStyle/01_filtN")
system("rm -r 02_DemultiplexedOut_MiSeqStyle/02_cutadapt")
